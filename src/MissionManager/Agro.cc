    ...
    , _entryPoint               (EntryLocationTopLeft)
{
    ...
    connect(&_exclusionAreaPolygon,     &QGCMapPolygon::polygonChanged,             this, &SurveyComplexItem::_rebuildTransects); // запертная зона 
...
void SurveyComplexItem::_rebuildTransectsPhase1WorkerSinglePolygon(bool refly)
{
    ...
    qCDebug(SurveyComplexItemLog) << "_rebuildTransectsPhase1 Allowed Polygon";
    ...
    if (_exclusionAreaPolygon.count() >= 3) {
        ...
        for (int i=0; i<_exclusionAreaPolygon.count(); i++) {
            ...
            qCDebug(SurveyComplexItemLog) << "_rebuildTransectsPhase1 Exclusion vertex:x:y" << vertex << exclusionPoints.last().x() << exclusionPoints.last().y();
        }
...
void SurveyComplexItem::_intersectLinesWithPolygonsAndHoles(const QList<QLineF>& lineList, const QPolygonF& mainPolygon, const QPolygonF& exclusionPolygon, QList<QLineF>& resultLines)
{
    resultLines.clear();
...
bool SurveyComplexItem::_containsPoint(const QPolygonF& polygon, const QPointF& point) const
{
    if (polygon.isEmpty()) {
        return false;
    }
