...
private:
    ...
    void _intersectLinesWithPolygonsAndHoles(const QList<QLineF>& lineList, const QPolygonF& mainPolygon, const QPolygonF& exclusionPolygon, QList<QLineF>& resultLines); // НОВЫЙ МЕТОД ДЛЯ ОБРЕЗКИ С УЧЕТОМ ЗАПРЕТНОЙ ЗОНЫ
    ...
    QGCMapPolygon   _exclusionAreaPolygon; // <<< НОВЫЙ ЧЛЕН: Запретная зона
    ...
    static constexpr const char* _jsonExclusionPolygonKey =   "exclusionPolygon"; // Новый ключ для запретной зоны
