...
class AgricultureComplexItem : public TransectStyleComplexItem
{
    Q_OBJECT
...
public:
    ...
    AgricultureComplexItem(PlanMasterController* masterController, bool flyView, const QString& kmlOrShpFile);
    ...
    // моё
    Q_PROPERTY(QGCMapPolygon* exclusionAreaPolygon READ exclusionAreaPolygon CONSTANT)
    ...
    static constexpr const char* jsonComplexItemTypeValue =   "agriculture";
    static constexpr const char* jsonV3ComplexItemTypeValue = "agriculture";

    static constexpr const char* settingsGroup =              "Agriculture";
...
private:
    ...
    // НОВЫЕ МЕТОДЫ ДЛЯ ЗАПРЕТНЫХ ЗОН
    bool _containsPoint(const QPolygonF& polygon, const QPointF& point) const;
    
    void _intersectLinesWithPolygonsAndHoles(const QList<QLineF>& lineList, 
                                             const QPolygonF& mainPolygon, 
                                             const QPolygonF& exclusionPolygon, 
